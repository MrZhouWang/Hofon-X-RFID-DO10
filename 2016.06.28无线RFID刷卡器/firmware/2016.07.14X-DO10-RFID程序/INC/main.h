...
MAIN_EXT volatile unsigned char  g_ucTempbuf[20]; //mrc522接收buffer
MAIN_EXT unsigned char  xdata DefaultKey[6];
...
typedef struct
{
	volatile UINT16 led_flash_count;			//递减计算器，系统在定时器中1MS自减1
	volatile UINT8  led_scan_flag;    		//led_Scan()==>LED扫描启动
}OS_SYS;
