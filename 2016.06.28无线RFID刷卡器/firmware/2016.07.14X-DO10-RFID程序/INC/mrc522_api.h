...
{
	 unsigned char SEC_KeyA[6];//扇区密码A
	 unsigned char SEC_KeyB[6];//扇区密码B
	 unsigned char SEC_FUC[4]; // 扇区功能码
}  ;



MRC522_API_EXT volatile unsigned short  rc_cont;//用来记录定时器读rc时间
MRC522_API_EXT volatile unsigned char  k_con;//记录卡离开时间，防止重刷机制
MRC522_API_EXT volatile unsigned char  kk_con;//记录卡离开时间，防止重刷机制
MRC522_API_EXT volatile unsigned char con_flag;//mrc522防冲突全局变量
...
#define LED_ON_TIME 5   //信号灯亮时间
#define MAXRLEN 18
#define MAX_K_CON 5		 //记录卡离开时间，防止重刷机制
#define RC_CON 10		 //记录卡/用来记录定时器读rc时间
