		...
		if (retCRC == usCRC) 						//crc校验成功 
		{
			...
			//	led_flash_NO = LED_GREEN;   
				...
				for(lightTowerIndex = 0; lightTowerIndex < 6; lightTowerIndex++)
				{
					...
					//亮灭周期控制数据获取
					LightTowerCtl[lightTowerIndex].led_flash_period = 1000;
					//亮灭比例控制数据获取
				    LightTowerCtl[lightTowerIndex].led_switch_ratio = LED_FLASH_MODE_1V5;//(1:on	10:off)
					//初始化状态、计数数据
