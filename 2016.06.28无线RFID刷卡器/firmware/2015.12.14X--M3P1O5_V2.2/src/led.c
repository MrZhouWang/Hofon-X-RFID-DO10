...
UINT8  const code LED_TBL[LED_TBL_MAX] = {
/*---------------------------------------------------------------------*/
//数码管显示0-9的七段码字节
//0-15，0--9,A-f
...
UINT16 GetLightTowerOnTime(int LightTowIndex)
{
	...
	{
	case 0X00:	break;				//常亮模式，不会调用此函数
	case 0X01:
	case 0X02:
	case 0X03:
	case 0X04:
		unRet = LightTowerCtl[LightTowIndex].led_flash_period;
		break;
	case 0X05:
		unRet = LightTowerCtl[LightTowIndex].led_flash_period*2;
		break;
	case 0X06:
		unRet = LightTowerCtl[LightTowIndex].led_flash_period*5;
		break;
	case 0X07:
		unRet = LightTowerCtl[LightTowIndex].led_flash_period*10;
		break;
	default:
		break;
	}
