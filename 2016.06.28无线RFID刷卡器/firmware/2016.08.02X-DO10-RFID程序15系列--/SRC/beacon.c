...
{ 
	...
	if(LightTowerCtl[0].led_status == ON)
	{
		...
		if(LightTowerCtl[0].led_switch_ratio == LED_FLASH_MODE_ALWAYS) //常亮
		{
	        CNTRL1_ON;
		}
		...
		{	
			...
			if(LightTowerCtl[0].led_status_cur == OFF)                   //当前灯为灭状态
			{
				
				if(LightTowerCtl[0].led_flash_count == 0)                  //灯灭计数器到，切换到灯亮状态，并且设置改变灯亮周期参数
				{
			        CNTRL1_ON;
			...
			else                                                         //当前为亮灯
			{
				...
				if(LightTowerCtl[0].led_flash_count == 0)                  //灯灭计数器到，切换到灯灭状态，并且设置改变灯灭周期参数
				{
			        CNTRL1_OFF;
	...
	if(LightTowerCtl[1].led_status == ON)
	{
		...
		if(LightTowerCtl[1].led_switch_ratio == LED_FLASH_MODE_ALWAYS)      //常亮
		{
	        CNTRL2_ON;
		}
